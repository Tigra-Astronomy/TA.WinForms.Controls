...
// 
// File: CadenceManager.cs  Last modified: 2016-09-12@23:15 by Tim Long
...
namespace TA.WinFormsControls
    ...
    ///     CadenceManager is intended primarily for Windows Forms controls, but can be used for any item that
    ///     implements the <see cref="ICadencedControl" /> interface.
    /// </summary>
    /// <remarks>
    ...
    public sealed class CadenceManager
        ...
        /// </summary>
        private static readonly List<ICadencedControl> UpdateList = new List<ICadencedControl>();
        ...
        private void TmrCadenceTick(object sender, ElapsedEventArgs e)
            ...
            foreach (var item in clonedUpdateList)
                {
                if (item == null)
                    continue;
                ...
                try
                    {
                    var cadenceMask = (uint) item.Cadence;
                    var state = cadenceMask.Bit(CadenceBitPosition);

                    if (control != null && control.InvokeRequired)
                        {
                        // It its a control and we're not on the GUI thread, marshall to the GUI thread.
                        control.Invoke(new CadenceUpdateDelegate(b => item.CadenceUpdate(b)), state);
                        }
                    else
                        {
                    ...
                    Log.Error(ex, "Collection possibly modified while iterating");
                    break;
                    }
                catch (Exception ex)
                    {
                    // If any managed item causes an exception of any other kind, remove it from the update list.
                    Log.Error(ex, "Exception while updating ICadencedControl - removing item from update list");
